...
Starting with `v1.4.3`, the application reads the download control's real HTTP(S) target and navigates through WebDriver without executing page scripts. This preserves session state and the configured download directory. Download-stage driver failures produce a concise diagnostic instead of placing JavaScript details such as `lastIndexOf` or a native ChromeDriver stack in the dialog and status bar. When opening an issue, include the application version, error code, and source hostname, but do not upload cookies, tokens, complete query URLs, or personal paths.
